...
test("loads default config if files don't exist", () => {
  ...
  expect(config.model).toBe("seeky-mini-latest");
...
test("loads user instructions + project doc when seeky.md is present", () => {
  // 1) seed memfs: a config JSON, an instructions.md, and a seeky.md in the cwd
  ...
  // and now our fake seeky.md in the cwd:
  const seekyPath = join(testDir, "seeky.md");
  memfs[seekyPath] = projectDoc;
