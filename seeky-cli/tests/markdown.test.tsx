...
- Handles special subcommands (e.g. \`seeky completion …\`), \`--config\`, API-key validation, then either:
  - Spawns the **AgentLoop** for the normal multi-step prompting/edits flow, or
  - Runs **single-pass** mode if \`--full-context\` is set.

`;
    const { lastFrame } = renderTui(
      <Markdown fileOpener={"vscode"} cwd="/home/user/seeky">
        {nestedList}
      </Markdown>,
    );
...
${BLUE}(${LINK_ON}vscode://file/home/user/seeky/src/cli.tsx:55${LINK_OFF})${COLOR_OFF}
    * Handles special subcommands (e.g. ${YELLOW}seeky completion …${COLOR_OFF}), ${YELLOW}--config${COLOR_OFF}, API-key validation, then
either:
