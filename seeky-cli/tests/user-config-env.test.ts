...
/**
 * Verifies that ~/.seeky.env is parsed (lowest‑priority) when present.
 */

describe("user‑wide ~/.seeky.env support", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = mkdtempSync(join(tmpdir(), "seeky-home-"));
    process.env["HOME"] = tempHome;

    // Ensure the env var is unset so that the file value is picked up.
    delete process.env["OPENAI_API_KEY"];

    // Write ~/.seeky.env with a dummy key.
    writeFileSync(
      join(tempHome, ".seeky.env"),
  ...
  it("loads the API key from ~/.seeky.env when not set elsewhere", async () => {
