#!/usr/bin/env node
// Unified entry point for the Seeky CLI.
/*
 * Behavior
 * =========
 ...
 *      environment variable SEEKY_RUST to a truthy value (`1`, `true`, etc.).
 ...
 *
 *      If the SEEKY_RUST=1 is specified and there is no native binary for the
...
// setting SEEKY_RUST=1 will opt-in to the native binary, if included.
const wantsNative = fs.existsSync(path.join(__dirname, "use-native")) ||
  (process.env.SEEKY_RUST != null
    ? ["1", "true", "yes"].includes(process.env.SEEKY_RUST.toLowerCase())
    : false);
...
if (wantsNative) {
  ...
  const binaryPath = path.join(__dirname, "..", "bin", `seeky-${targetTriple}`);
