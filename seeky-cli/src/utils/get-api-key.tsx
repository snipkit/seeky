    ...
    if (!idClaims || !idClaims.exp || Date.now() >= idClaims.exp * 1000) {
      ...
      try {
        ...
        if (!refreshRes.ok) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            `Please sign in again to redeem credits: ${chalk.bold("seeky --login")}`,
          );
          return;
        }
...
async function handleCallback(
  ...
  const exchangeParams = new URLSearchParams({
    ...
    name: `Seeky CLI [auto-generated] (${new Date().toISOString().slice(0, 10)}) [${
      randomId
    }]`,
  });
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Sign into Seeky CLI</title>
    ...
    <div class="container">
      <div class="inner-container">
        <div class="content">
          ...
          <div class="title">Signed in to Seeky CLI</div>
        </div>
