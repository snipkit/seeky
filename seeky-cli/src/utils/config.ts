...
// ---------------------------------------------------------------------------
// User‑wide environment config (~/.seeky.env)
// ---------------------------------------------------------------------------
...
//   3. User‑wide ~/.seeky.env (loaded here)
...
const USER_WIDE_CONFIG_PATH = join(homedir(), ".seeky.env");
...
export const DEFAULT_AGENTIC_MODEL = "seeky-mini-latest";
...
export const CONFIG_DIR = join(homedir(), ".seeky");
...
// Can be set `true` when Seeky is running in an environment that is marked as already
// considered sufficiently locked-down so that we allow running without an explicit sandbox.
export const SEEKY_UNSAFE_ALLOW_NO_SANDBOX = Boolean(
  process.env["SEEKY_UNSAFE_ALLOW_NO_SANDBOX"] || "",
);
...
// ---------------------------------------------------------------------------
// Project doc support (AGENTS.md / seeky.md)
// ---------------------------------------------------------------------------
...
// the legacy `seeky.md` variants for backwards-compatibility so that existing
 ...
 * `seeky.md`) if present. If the file
...
export function loadProjectDoc(cwd: string, explicitPath?: string): string {
  ...
  if (explicitPath) {
    ...
    if (!existsSync(filepath)) {
      ...
      console.warn(`seeky: project doc not found at ${filepath}`);
      filepath = null;
    }
  } else {
  ...
  try {
    ...
    if (buf.byteLength > PROJECT_DOC_MAX_BYTES) {
      // eslint-disable-next-line no-console
      console.warn(
        `seeky: project doc '${filepath}' exceeds ${PROJECT_DOC_MAX_BYTES} bytes – truncating.`,
      );
    }
...
export const loadConfig = (
  ...
  if (
    ...
    if (storedConfig.disableResponseStorage === "true") {
      ...
      log(
        `[seeky] Warning: 'disableResponseStorage' in config is not a boolean (got '${storedConfig.disableResponseStorage}'). Ignoring this value.`,
      );
  ...
  const shouldLoadProjectDoc =
    ...
    process.env["SEEKY_DISABLE_PROJECT_DOC"] !== "1";
  ...
  if (shouldLoadProjectDoc) {
    ...
    if (projectDocPath) {
      log(
        `[seeky] Loaded project doc from ${projectDocPath} (${projectDoc.length} bytes)`,
      );
    } else {
      log(`[seeky] No project doc found in ${cwd}`);
    }
  }
  ...
  // materialised ~/.seeky/config.json file on first execution.  This mirrors
  ...
  try {
    if (!existsSync(actualConfigPath)) {
      ...
      // pinning users to an old default after upgrading Seeky.
      const ext = extname(actualConfigPath).toLowerCase();
      if (ext === ".yaml" || ext === ".yml") {
    ...
    // block the CLI from starting.  A future explicit `seeky config` command
