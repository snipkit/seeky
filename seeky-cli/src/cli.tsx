...
if (major < 22) {
  ...
  console.error(
    "\n" +
      "Seeky CLI requires Node.js version 22 or newer.\n" +
...
const cli = meow(
  `
  Usage
    $ seeky [options] <prompt>
    $ seeky completion <bash|zsh|fish>

  Options
    --version                       Print version and exit

    -h, --help                      Show usage and exit
    -m, --model <model>             Model to use for completions (default: seeky-mini-latest)
  ...
  Examples
    $ seeky "Write and run a python program that prints ASCII art"
    $ seeky -q "fix build issues"
    $ seeky completion bash
`,
  {
    ...
    flags: {
      ...
      approvalMode: {
        ...
        aliases: ["a"],
        description:
          "Determine the approval mode for Seeky (default: suggest) Values: suggest, auto-edit, full-auto",
      },
      writableRoot: {
        type: "string",
        isMultiple: true,
...
if (cli.input[0] === "completion") {
  ...
  const scripts: Record<string, string> = {
    bash: `# bash completion for seeky
_seeky_completion() {
  local cur
  cur="\${COMP_WORDS[COMP_CWORD]}"
  COMPREPLY=( $(compgen -o default -o filenames -- "\${cur}") )
}
complete -F _seeky_completion seeky`,
    zsh: `# zsh completion for seeky
#compdef seeky
...
_seeky`,
    fish: `# fish completion for seeky
complete -c seeky -a '(__fish_complete_path)' -d 'file path'`,
  };
...
try {
  ...
  const authDir = path.join(home, ".seeky");
...
if (cli.flags.free) {
  // eslint-disable-next-line no-console
  console.log(`${chalk.bold("seeky --free")} attempting to redeem credits...`);
...
if (cli.flags.quiet) {
  process.env["SEEKY_QUIET_MODE"] = "1";
  if (!prompt || prompt.trim() === "") {
    // eslint-disable-next-line no-console
    console.error(
      'Quiet mode requires a prompt string, e.g.,: seeky -q "Fix bug #123 in the foobar project"',
    );
    process.exit(1);
  }
...
// 4. config.approvalMode - use the approvalMode setting from ~/.seeky/config.json.
