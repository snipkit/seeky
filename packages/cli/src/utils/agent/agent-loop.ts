...
// See https://github.com/khulnasoft/openai-node/tree/v4?tab=readme-ov-file#configuring-an-https-agent-eg-for-proxies
      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            if (this.model.startsWith("o") || this.model.startsWith("seeky")) {
...
const prefix = `You are operating as and within the Seeky CLI, a terminal-based agentic coding assistant built by OpenAI. It wraps OpenAI models to enable natural language interaction with a local codebase. You are expected to be precise, safe, and helpful.
...
- More details on your functionality are available at \`seeky --help\`

The Seeky CLI is open-sourced. Don't confuse yourself with the old Seeky language model built by OpenAI many moons ago (this is understandably top of mind for you!). Within this context, Seeky refers to the open-source agentic coding interface.
