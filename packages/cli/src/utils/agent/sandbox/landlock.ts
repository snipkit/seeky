...
The combination of seccomp/landlock that Seeky uses for sandboxing is not
...
sandboxing guarantees and then run Seeky with the
 ...
 *
 * Running `seeky-linux-sandbox-x64 -- true` in the container fails with:
 *
 * ```
...
function getLinuxSandboxExecutableForCurrentArchitecture(): string {
  switch (process.arch) {
    case "arm64":
      return "seeky-linux-sandbox-arm64";
    case "x64":
      return "seeky-linux-sandbox-x64";
