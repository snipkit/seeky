...
/// Helper that consolidates the common boilerplate found in several Seeky
/// binaries (`seeky`, `seeky-exec`, `seeky-tui`) around dispatching to the
/// `seeky-linux-sandbox` sub-command.
///
/// When the current executable is invoked through the hard-link or alias
/// named `seeky-linux-sandbox` we *directly* execute [`run_main`](crate::run_main)
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    if exe_name == "seeky-linux-sandbox" {
    ...
    runtime.block_on(async move {
        let seeky_linux_sandbox_exe: Option<PathBuf> = if cfg!(target_os = "linux") {
            std::env::current_exe().ok()
        } else {
            None
        };

        main_fn(seeky_linux_sandbox_exe).await
    })
}

#[cfg(not(target_os = "linux"))]
pub fn run_main() -> ! {
    panic!("seeky-linux-sandbox is only supported on Linux");
}
