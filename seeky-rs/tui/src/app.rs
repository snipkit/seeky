...
enum AppState {
    ...
    Chat,
    /// The start‑up warning that recommends running seeky inside a Git repo.
...
impl App<'_> {
    ...
    pub(crate) fn run(&mut self, terminal: &mut tui::Tui) -> Result<()> {
        ...
        while let Ok(event) = self.app_event_rx.recv() {
            match event {
                ...
                AppEvent::SeekyEvent(event) => {
                    self.dispatch_seeky_event(event);
                }
    ...
    fn dispatch_seeky_event(&mut self, event: Event) {
        if matches!(self.app_state, AppState::Chat) {
            if let Err(e) = self.chat_widget.handle_seeky_event(event) {
                tracing::error!("SendError: {e}");
            }
        }
    }
