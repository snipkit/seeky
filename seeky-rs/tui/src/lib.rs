...
// The standalone `seeky-tui` binary prints a short help message before the
...
pub fn run_main(cli: Cli, seeky_linux_sandbox_exe: Option<PathBuf>) -> std::io::Result<()> {
    ...
    // Seeky CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("seeky-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for seeky crates.
    let env_filter = || {
        EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("seeky_core=info,seeky_tui=info"))
    };
