...
impl ChatWidget<'_> {
    pub(crate) fn new(
        ...
        tokio::spawn(async move {
            ...
            app_event_tx_clone.send(AppEvent::SeekyEvent(session_event.clone()));
            ...
            while let Ok(event) = seeky.next_event().await {
                app_event_tx_clone.send(AppEvent::SeekyEvent(event));
            }
        });
    ...
    pub(crate) fn handle_seeky_event(&mut self, event: Event) {
        ...
        match msg {
            ...
            EventMsg::GetHistoryEntryResponse(event) => {
                let seeky_core::protocol::GetHistoryEntryResponseEvent {
                    offset,
                    log_id,
                    entry,
                } = event;
