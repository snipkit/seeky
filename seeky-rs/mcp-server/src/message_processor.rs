...
impl MessageProcessor {
    ...
    fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "seeky-mcp-server".to_string(),
    ...
    fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![create_tool_for_seeky_tool_call_param()],
            next_cursor: None,
        };
    ...
    fn handle_call_tool(
        ...
        // We only support the "seeky" tool for now.
        if name != "seeky" {
        ...
        let (initial_prompt, config): (String, SeekyConfig) = match arguments {
            Some(json_val) => match serde_json::from_value::<SeekyToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.seeky_linux_sandbox_exe.clone()) {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![CallToolResultContent::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load Seeky configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Seeky tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                    };
            ...
            None => {
                let result = CallToolResult {
                    content: vec![CallToolResultContent::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for seeky tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        // Spawn an async task to handle the Seeky session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Seeky session and stream events back to the client.
            crate::seeky_tool_runner::run_seeky_tool_session(id, initial_prompt, config, outgoing)
                .await;
        });
    }
