...
async fn spawn_seeky() -> Result<Seeky, SeekyErr> {
    assert!(
        api_key_available(),
...
async fn live_streaming_and_prev_id_reset() {
    ...
    let seeky = spawn_seeky().await.unwrap();
    ...
    loop {
        let ev = timeout(Duration::from_secs(60), seeky.next_event())
            .await
...
async fn live_shell_function_call() {
    ...
    const MARKER: &str = "seeky_live_echo_ok";
    ...
    loop {
        ...
        match ev.msg {
            EventMsg::ExecCommandBegin(seeky_core::protocol::ExecCommandBeginEvent {
                command,
                call_id: _,
                cwd: _,
            }) => {
            ...
            EventMsg::ExecCommandEnd(seeky_core::protocol::ExecCommandEndEvent {
                stdout,
                exit_code,
                call_id: _,
                stderr: _,
