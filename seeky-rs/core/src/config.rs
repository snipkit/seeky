...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Seeky will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Seeky
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.seeky/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Seeky"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Seeky '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// Definition for MCP servers that Seeky can reach out to for tool calls.
...
impl Config {
    ...
    fn load_instructions(seeky_dir: Option<&Path>) -> Option<String> {
        let mut p = match seeky_dir {
