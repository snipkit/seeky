...
use seeky_apply_patch::AffectedPaths;
use seeky_apply_patch::ApplyPatchAction;
use seeky_apply_patch::ApplyPatchFileChange;
use seeky_apply_patch::MaybeApplyPatchVerified;
use seeky_apply_patch::maybe_parse_apply_patch_verified;
use seeky_apply_patch::print_summary;
use futures::prelude::*;
use serde::Serialize;
use serde_json;
...
/// The high-level interface to the Seeky system.
...
impl Seeky {
    /// Spawn a new [`Seeky`] and initialize the session. Returns the instance
    /// of `Seeky` and the ID of the `SessionInitialized` event that was
    /// submitted to start the session.
    pub async fn spawn(config: Config, ctrl_c: Arc<Notify>) -> SeekyResult<(Seeky, String)> {
        ...
        let init_id = seeky.submit(configure_session).await?;

        Ok((seeky, init_id))
    }

    /// Submit the `op` wrapped in a `Submission` with a unique ID.
    pub async fn submit(&self, op: Op) -> SeekyResult<String> {
        let id = self
            .next_id
    ...
    /// Use sparingly: prefer `submit()` so Seeky is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> SeekyResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| SeekyErr::InternalAgentDied)?;
        Ok(())
    }

    pub async fn next_event(&self) -> SeekyResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
async fn run_turn(
...
) -> SeekyResult<Vec<ProcessedResponseItem>> {
    ...
    loop {
        match try_run_turn(sess, &sub_id, &prompt).await {
            ...
            Err(SeekyErr::Interrupted) => return Err(SeekyErr::Interrupted),
            Err(SeekyErr::EnvVar(var)) => return Err(SeekyErr::EnvVar(var)),
            Err(e) => {
